...
class Level:
				...
				for col_index, col in enumerate(row):
					if col != '-1':
						...
						if style == 'grass':
							...
							Tile(
								(x,y), 
								[self.visible_sprites,self.obstacle_sprites,self.attackable_sprites],	# grass is also attackable (self_attackable_sprites)
								'grass',
								random_grass_image)
						if style == 'object':
	...
	def run(self):
		# update and draw the game / calling it
...
class YsortCameraGroup(pygame.sprite.Group):
	...
	def enemy_update(self,player):
		enemy_sprites = [sprite for sprite in self.sprites() if hasattr(sprite,'sprite_type') and sprite.sprite_type == 'enemy']	#hasattr checks a class: (sprite) then a specific attribute:(sprite_type)
